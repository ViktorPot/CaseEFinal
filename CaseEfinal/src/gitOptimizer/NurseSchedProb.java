...
public class NurseSchedProb extends OptimizationProblem {
    ...
    /**
     * Constructs a Nurse Scheduling Problem. In this problem, maxShiftsInRow
     * and maxShiftsADay are treated as hard constraints. minShifts, however,
     * has the option of being ignored via a lambdaMin value of zero.
     */
    public NurseSchedProb(int numEmployees, int numDays, int numShifts, int maxShiftsInRow,
        ...
        // checking user input
        if (numEmployees <= 0) {
            throw new PositiveNumberInputException("number of employees");
        }
        if (numDays <= 0) {
            throw new PositiveNumberInputException("number of days in scheduling cycle");
        }
        if (numShifts <= 0) {
            throw new PositiveNumberInputException("number of shifts per day");
        }
        if (maxShiftsInRow <= 0) {
            throw new PositiveNumberInputException("maximum number of shifts in a row");
        }
        if (maxShiftsADay <= 0) {
            throw new PositiveNumberInputException("maximum number of shifts per 24 hours");
        }
        if (minShifts < 0) {
            throw new PositiveNumberInputException("minimum number of shifts for an employee per scheduling period");
        }
        ...
        if (lambdaPref < 0) {
            throw new PositiveNumberInputException("the proportional weight of satisfying employee preferences");
        }
        if (lambdaPref > 1) {
            throw new InputException("the proportional weight of satisfying employee preferences",
                    "must be less than or equal to one", "enter a value between zero and one, inclusive");
        }
        if (lambdaMin < 0) {
            throw new PositiveNumberInputException("the proportional weight of satisfying minimum shifts per employee per scheduling period");
        }
        if (lambdaMin > 1) {
            throw new InputException("the proportional weight of satisfying minimum shifts per employee per scheduling period",
                    "must be less than or equal to one", "enter a value between zero and one, inclusive");
        }
        ...
        for (int i = 0; i < numEmployees; i++) { // for each employee
            ...
            for (int j = 0; j < length; j++) { // for each shift
                ...
                for (int k = 0; k < maxShiftsInRow + 1; k++) {
                    if (schedule.get((j + k) % length) == 0) {
                        break;
                    } else {
                        counter++;
                    }
                }
                if (counter > maxShiftsInRow) {
                    violations += penalty;
                }
            }
        }
    ...
    /**
     * A solution is within constraints if its number of workers per shift match
